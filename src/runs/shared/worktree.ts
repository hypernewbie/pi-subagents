...
function resolveRepoState(cwd: string): RepoState {
	...
	// Legacy project-local state remains ignorable while users migrate it.
	const status = runGitChecked(toplevel, ["status", "--porcelain", "--", `:!${LEGACY_PROJECT_SUBAGENTS_RELATIVE_DIR}`]);
