...
function discoverHandoffPaths(repoRoot: string, input: BuildWorktreeCleanupPlanInput): { paths: string[]; warnings: string[] } {
	...
	const artifactsDir = path.join(repoRoot, LEGACY_PROJECT_SUBAGENTS_RELATIVE_DIR, "artifacts");
...
export function worktreeCleanupPlanPath(repoRoot: string, planId: string): string {
	return path.join(repoRoot, LEGACY_PROJECT_SUBAGENTS_RELATIVE_DIR, "cleanup-plans", `${validatePlanId(planId)}.json`);
}
